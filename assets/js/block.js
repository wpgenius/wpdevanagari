...
wp.blocks.registerBlockType("zozuk/wphindi", {
  ...
  edit: function(props) {
    return new MarathiEditor(props);
  },
  save: function(props) {
...
class MarathiEditor extends React.Component {
  constructor(props) {
    super(props);
    this.elementReference = React.createRef();
    this.Id = "zz-" + props.clientId;
    const editorID = `.${this.Id}`;
    this.hindiTyping = new transliterator(editorID);
  }
  render() {
    ...
    return [
      ...
      React.createElement(wp.blockEditor.RichText, {
        ...
        id: "zz-" + Date.now(),
        placeholder: "Start typing in devanagari",
        onChange: function(content) {
          ...
          if (isEnter) {
            ...
            contentArray.forEach((content,index)=>{
              ...
              const WPMarathiBlock = wp.blocks.createBlock('zozuk/wphindi',{content});
              props.insertBlocksAfter(WPMarathiBlock);
            });
  ...
  toggleButton() {
    ...
    return React.createElement(
      "button",
      {
        className: "wp-devanagari-block-editor-button active",
        onClick: function(event) {
          ...
          if (!button.classList.contains("disabled")) {
            text.innerText = "Enable WPDevanagari";
          } else {
            text.innerText = "Disable WPDevanagari";
          }
      ...
      [
        React.createElement("span", {
          className: "wpmarathi-logo active"
        }),
        React.createElement(
          "span",
          {
...
jQuery("body").on("keydown", ".wp-block-wpgenius-WPDevanagari", function(e) {
  if (e.key == "Enter") {
    isEnter = true;
  } else {
    isEnter = false;
